...
`use_style()` applies high-DPI, retina-friendly matplotlib defaults and the
`PALETTE`, and `style_table(df, ...)` renders a clean, consistently-formatted
pandas table. Reuse it in your own work: `from dpspice.plotting import
use_style, PALETTE, style_table`.
